...
export const fetchPostsSummary = async (pageNo, tagSlug, maxPostsPerPage = POSTS_PER_PAGE) => {
    const contentfulClient = getContenfulClient()

    // Query statement
    const query = {
        limit: POSTS_PER_PAGE,
        skip: pageNo * POSTS_PER_PAGE,
        content_type: 'blogPost',
        order: ['-sys.updatedAt', '-sys.createdAt'],
        select: ['sys.updatedAt', 'fields.title', 'fields.slug', 'fields.tags', 'fields.keywords', 'fields.description', 'fields.postedOn', 'fields.heroImage', 'fields.heroImageBlur'],
        include: 10
    }
    ...
    try {
        ...
        const blogPostsData = {
            ...
            blogPosts: blogPostsContentful.items.map((blogPostData) => {
                const { title, slug, tags, keywords, description, postedOn, heroImage, heroImageBlur } = blogPostData.fields
