...
const ScrollbarAnimatedStyle = createGlobalStyle`
    ...
    ::-webkit-scrollbar-thumb {
      background: ${({ topColor, bottomColor }) => (`linear-gradient(135deg, ${topColor},${bottomColor})`)};
      border-radius: 12px;
    }
    ::-webkit-scrollbar-thumb:hover {
      background: ${({ topColorHover, bottomColorHover }) => (`linear-gradient(135deg, ${topColorHover}, ${bottomColorHover})`)};
    }

    * {
        
        scrollbar-color: ${({ topColor, bottomColor, theme }) => (`linear-gradient(135deg, ${topColor},${bottomColor}) ${theme.backgroundColor}`)};
    }
`
...
export const ScrollbarAnimated = () => {
  ...
  const breakPoints = [0, 0.33333333, 0.6666666]
